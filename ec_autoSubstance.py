...
class AutoSubstance():
    ...
    def createWin(self):
        ...
        self.win = cmds.window(self.win_name, w=100, h=400, s=0, t='EC Auto Substance')
        ...
        self.create_btn = cmds.button(h=20, l='create network', c=self.build)
    ...
    def build(self, *args):
        ...
        for item in sel:
            if cmds.objectType(item) == 'file':
                ...
                file_name = file_name_long.split('/')[-1].split('.')[0]
        ...
        # build the shader
        mat = cmds.shadingNode('aiStandardSurface', asShader=1, n=texture_set+'_MAT')
        cmds.setAttr(mat+'.base', 1)

        sg = cmds.sets(renderable=1, noSurfaceShader=1, empty=1, n=texture_set+'_SG')
        ...
        if emission_map:
            cmds.connectAttr(emission_map+'.outColor', mat+'.'+emission_attr)
        ...
        if height_map and cmds.checkBox(self.disp_box, q=1, v=1):
            ...
            cmds.connectAttr(disp+'.displacement', sg+'.displacementShader')
        if normal_map:
            bump = aiNormal(texture_set, normal_map)
            cmds.connectAttr(bump+'.outNormal', mat+'.'+normal_attr)
        if metal_map:
        ...
        if cmds.checkBox(self.sss_box, q=1, v=1):
            ...
            cmds.connectAttr(remap_hsv+'.outColor', mat+'.subsurfaceRadius')
...
def facingRatio(name, texture):
    ...
    cmds.connectAttr(color_correct+'.outColor', ramp+'.colorEntryList[0].color')
...
def aiNormal(name, texture):
    ...
    bump = cmds.shadingNode('bump2d', au=1, n=name+'_B2D')
    cmds.connectAttr(texture+'.outAlpha', bump+'.bumpValue')
    cmds.setAttr(bump+'.bumpInterp', 1)
    cmds.setAttr(bump+'.aiFlipR', 0)
    cmds.setAttr(bump+'.aiFlipG', 0)
